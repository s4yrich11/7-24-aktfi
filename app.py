...
# --- LOG KANALI ID AYARI ---
# Sunucunda oluşturduğun gizli log kanalının ID'sini buraya tam olarak yapıştır:
...
# İzin vermek istediğin kişilerin Discord ID'lerini buraya ekle
YETKILI_IDLERI = [852839991297966112]  # Kendi ID'ni ve izin vermek istediklerini buraya yaz
...
async def leaderboard_command(ctx):
  ...
  # Kullanıcıları toplam XP'lerine göre büyükten küçüğe sıralıyoruz
  sorted_users = sorted(
      xp_data.items(), key=lambda item: item[1]["xp"], reverse=True
  )

  # En yüksek ilk 10 kullanıcıyı alalım
  ...
  for index, (user_id, data) in enumerate(top_users, start=1):
    # Kullanıcı ID'sini Discord nesnesine dönüştürmeye çalışıyoruz
    member = ctx.guild.get_member(int(user_id))
    name = member.name if member else f"Kullanıcı ({user_id})"

    # Derecelere göre özel emojiler
