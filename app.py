...
# Buton ve rol mantığını yöneten sınıf
class RoleButtonView(discord.ui.View):

  def __init__(self, role_id: int):
    super().__init__(
        timeout=None
    )  # Timeout None olunca bot yeniden başlasa bile buton çalışmaya devam eder
  ...
  async def role_button_callback(
    ...
    if not role:
      await interaction.response.send_message(
          "Hata: Rol bulunamadı veya silinmiş!", ephemeral=True
      )
      return

    member = interaction.user

    # Kullanıcıda rol varsa al, yoksa ver
    if role in member.roles:
      await member.remove_roles(role)
      await interaction.response.send_message(
          f"Başarıyla **{role.name}** rolü üzerinizden alındı!", ephemeral=True
      )
    else:
      await member.add_roles(role)
      await interaction.response.send_message(
          f"Başarıyla **{role.name}** rolü size verildi!", ephemeral=True
      )
...
class ReactionRoleCog(commands.Cog):
  ...
  @commands.command(name="rolkur", help="Belirtilen rol için butonlu mesaj gönderir.")
  @commands.has_permissions(administrator=True)
  async def rolkur(self, ctx, role: discord.Role, *, mesaj_metni: str = None):
    if not mesaj_metni:
      mesaj_metni = (
          f"Aşağıdaki butona tıklayarak **{role.name}** rolünü alabilir veya"
          " bırakabilirsin!"
      )
    ...
    embed = discord.Embed(
        title="Rol Seçim Paneli", description=mesaj_metni, color=discord.Color.blurple()
    )

    await ctx.send(embed=embed, view=view)
    # Komut mesajını ortalığı kirletmesin diye silebilirsin (isteğe bağlı)
    try:
      await ctx.message.delete()
    except:
      pass
