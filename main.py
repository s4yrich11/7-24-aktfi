...
# --- YENİ EKLENEN ROL VERME VE ALMA KOMUTLARI ---

@bot.command()
@commands.has_permissions(manage_roles=True)
async def rolver(ctx, member: discord.Member, *, role: discord.Role):
    """Belirtilen kullanıcıya bir rol verir. Örn: !rolver @Kullanici 18+"""
...
async def rolal(ctx, member: discord.Member, *, role: discord.Role):
    """Belirtilen kullanıcıdan bir rol alır. Örn: !rolal @Kullanici 18+"""
...
async def rol_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send("Bu komutu kullanmak için `Rolleri Yönet` yetkisine sahip olmalısın.")
    elif isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("Eksik argüman! Kullanım: `!rolver @Kullanici RolAdı` veya `!rolal @Kullanici RolAdı`")
    elif isinstance(error, commands.RoleNotFound):
        await ctx.send("Belirtilen rol bulunamadı. Lütfen rol adını doğru yazdığınızdan emin olun.")
