...
# --- METİN KANALI İŞLEMLERİ ---

@bot.command()
async def metinsustur(ctx, member: discord.Member, *, neden="Belirtilmedi"):
    """Kullanıcıya 'Susturuldu' rolü vererek metin kanallarında susturur."""
    if ctx.author.id != SAHIP_ID:
        await ctx.send("Bu komutu sadece botun sahibi kullanabilir!")
        return
    ...
    if not role:
        await ctx.send("Sunucuda 'Susturuldu' adında bir rol bulunamadı. Lütfen önce bu isimde bir rol oluşturup metin kanallarından mesaj gönderme iznini kapatın.")
        return

    if role in member.roles:
        await ctx.send(f"{member.mention} zaten metin kanallarında susturulmuş.")
        return

    try:
        await member.add_roles(role, reason=neden)
        await ctx.send(f"💬✍️ **{member.display_name}** metin kanallarında susturuldu. Neden: `{neden}`")
    except discord.Forbidden:
        await ctx.send("Bu kullanıcıya 'Susturuldu' rolünü vermek için yeterli yetkim yok.")
    except discord.HTTPException as e:
        await ctx.send(f"Bir hata oluştu: {e}")
...
async def metinsusturac(ctx, member: discord.Member):
    ...
    role = discord.utils.get(ctx.guild.roles, name="Susturuldu")
    if not role:
        await ctx.send("Sunucuda 'Susturuldu' adında bir rol bulunamadı.")
        return
    ...
    try:
        ...
        await ctx.send("Kullanıcıdan 'Susturuldu' rolünü almak için yeterli yetkim yok.")
    except discord.HTTPException as e:
        await ctx.send(f"Bir hata oluştu: {e}")
