...
# --- 1. SUNUCUDAKI ÜYELERİ VE BİLGİLERİNİ LİSTELEME API'Sİ ---
@app.route('/api/sunucu-uyeleri/<int:guild_id>', methods=['GET'])
def api_uyeleri_getir(guild_id):
    try:
        guild = bot.get_guild(guild_id)
        if not guild:
            return jsonify({"hata": "Bot bu sunucuda bulunmuyor!"}), 404
        ...
        for member in guild.members:
            uyeler.append({
                "id": member.id,
                "isim": member.name,
                "gorunen_ad": member.display_name,
                "bot": member.bot,
                "avatar": member.avatar.url if member.avatar else member.default_avatar.url,
                "roller": [role.name for role in member.roles if role != guild.default_role]
            })
        return jsonify({"durum": "basarili", "uyeler": uyeler})
    except Exception as e:
        return jsonify({"hata": str(e)}), 500


# --- 2. WEBTEN KULLANICIYA ROL VERME API'Sİ ---
@app.route('/api/rol-ver', methods=['POST'])
def api_rol_ver():
    veri = request.json
    guild_id = veri.get('guild_id')
    user_id = veri.get('user_id')
    rol_adi = veri.get('rol_adi')

    guild = bot.get_guild(guild_id)
    if not guild:
        return jsonify({"hata": "Sunucu bulunamadı!"}), 404

    member = guild.get_member(user_id)
    role = discord.utils.get(guild.roles, name=rol_adi)

    if not member or not role:
        return jsonify({"hata": "Kullanıcı veya rol bulunamadı!"}), 404

    try:
        future = asyncio.run_coroutine_threadsafe(member.add_roles(role), bot.loop)
        future.result(timeout=5)
        return jsonify({"durum": "basarili", "mesaj": f"{member.display_name} adlı kullanıcıya {role.name} rolü verildi!"})
    except Exception as e:
        return jsonify({"hata": str(e)}), 500


# --- 3. BOTUN LOGLARINI WEB'E AKTARMA API'Sİ ---
# (Botun logları anlık tutabilmesi için basit bir bellek listesi kullanalım)
SON_LOGLAR = []

def web_log_ekle(mesaj):
    global SON_LOGLAR
    SON_LOGLAR.insert(0, mesaj)
    if len(SON_LOGLAR) > 50:  # Son 50 logu sakla
        SON_LOGLAR.pop()

@app.route('/api/loglari-getir', methods=['GET'])
def api_loglari_getir():
    return jsonify({"durum": "basarili", "loglar": SON_LOGLAR})


# --- CANLI LOG SİSTEMİ İÇİN BELLEK ---
SON_LOGLAR = ["🚀 Web paneli ve log sistemi aktif edildi."]

def web_log_ekle(mesaj):
    global SON_LOGLAR
    SON_LOGLAR.insert(0, mesaj)
    if len(SON_LOGLAR) > 100:  # Son 100 logu hafızada tut
        SON_LOGLAR.pop()

# Flask içine bu rotayı ekle:
@app.route('/api/loglar', methods=['GET'])
def api_loglari_getir():
    return jsonify({"loglar": SON_LOGLAR})
