...
@bot.command()
async def oynat(ctx, *, aranan: str = None):
    """Ses kanalına katılır ve müziği oynatır."""
    if not ctx.author.voice:
        await ctx.send("Önce bir ses kanalına girmelisin!")
        return
    
    if not aranan:
        await ctx.send("Lütfen çalınacak bir şarkı adı veya link yaz. Örn: `!oynat nafile`")
        return
    ...
    try:
        ...
        await ctx.send(f"🎶 **{aranan}** için ses bağlantısı kuruldu! (Müzik akışı aktif)")
    except Exception as e:
        await ctx.send(f"Hata oluştu: `{e}`")
