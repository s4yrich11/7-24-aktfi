...
async def on_ready():
    ...
    # Bot açıldığında otomatik olarak Rahatsız Etmeyin (DND) ve belirtilen aktiviteyi yapar
...
async def metinsustur(ctx, member: discord.Member, *, neden="Belirtilmedi"):
    ...
    try:
        ...
        await ctx.send("❌ Hata: Bu kullanıcıya rol vermek için yetkim yok! (Botun rolü hedef kullanıcının rolünden aşağıda olabilir veya 'Rolleri Yönet' yetkisi eksik).")
    except discord.HTTPException as e:
        await ctx.send(f"Bir hata oluştu: {e}")
