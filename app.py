...
async def kurucu_veya_sahip(ctx):
    # Botun sahibi mi veya sunucunun kurucusu mu kontrol eder
    is_owner = await ctx.bot.is_owner(ctx.author)
    is_guild_owner = ctx.guild and ctx.author == ctx.guild.owner
    return is_owner or is_guild_owner

@bot.command()
@commands.check(852839991297966112)
