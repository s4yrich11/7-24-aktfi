...
# yt_dlp ayarları (Arama yapabilmesi için ytsearch ekliyoruz)
ytdl_format_options = {
    'format': 'bestaudio/best',
    'noplaylist': True,
    'default_search': 'ytsearch', # Link yerine isim yazıldığında YouTube'da aratır
}
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def from_url(cls, search, *, loop=None):
        ...
        # Arama terimini veya linki işleme sokuyoruz
        data = await loop.run_in_executor(None, lambda: ytdl.extract_info(search, download=False))
        
        if 'entries' in data:
            # Eğer arama yapıldıysa çıkan ilk sonucu alır
            data = data['entries'][0]
...
async def cal(ctx, *, arama: str):
    ...
    # Kullanıcıya arandığını bildiren geçici mesaj atabilirsin
...
@bot.command(name="çık", help="Botu ses kanalından çıkarır")
async def cik(ctx):
    if ctx.voice_client:
        await ctx.voice_client.disconnect()
        await ctx.send("Ses kanalından ayrıldım.")
